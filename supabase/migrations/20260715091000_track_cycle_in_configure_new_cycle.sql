...
declare
  ...
  v_paraguay_date date := (
    current_timestamp at time zone 'America/Asuncion'
  )::date;
  v_current_cycle text :=
    to_char(v_paraguay_date, 'YYYY')
    || '-'
    || case
      when extract(month from v_paraguay_date)::integer <= 6 then '1'
      else '2'
    end;
...
begin
  ...
  -- Marca el ciclo únicamente después de completar el reemplazo activo.
  ...
  set
    ...
    last_configured_cycle = v_current_cycle,
    updated_at = now()
  where user_id = v_user_id;
...
is
  'Reemplaza atómicamente los datos activos del ciclo académico del usuario autenticado y registra el ciclo vigente de Paraguay sin modificar notas finales ni materias aprobadas.';
